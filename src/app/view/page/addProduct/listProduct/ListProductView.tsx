...
const vm: ListProductViewModel = container.get<ListProductViewModel>("ListProductViewModel");
...
console.log(keyword);
    return (
        <BackGround>
            ...
            <Container>
            <Wrap>
                <input value={keyword} /*onBlur={() => {setKeyword("");}}*/ onChange={(e)=> {setKeyword(e.target.value);}}/>
                {keyword ? (
                  <SearchedBox>
                    {products.filter((product)=> product.title.includes(keyword))
                    .map((item) => {return <SearchedItem onClick={()=>{setKeyword(item.title);}} key={item.id}>{item.title}</SearchedItem>;})}
                  </SearchedBox>) : (null)}
